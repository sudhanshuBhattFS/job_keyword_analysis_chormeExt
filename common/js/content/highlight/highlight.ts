export interface KeywordMatchResult {
    whitelistMatched: string[];
    blacklistMatched: string[];
}

export function highlightAndCollectKeywords(
    ...
    // Remove previously applied highlights
    elements.forEach((el) => {
    ...
    elements.forEach((el) => {
        ...
        textNodes.forEach((textNode) => {
            ...
            const whiteMatches =
                originalText.match(whitelistRegex ?? /$^/) || [];
            const blackMatches =
                originalText.match(blacklistRegex ?? /$^/) || [];

            whiteMatches.forEach((word) =>
                whitelistMatched.add(word.toLowerCase())
            );
            blackMatches.forEach((word) =>
                blacklistMatched.add(word.toLowerCase())
            );
    ...
    return {
        whitelistMatched: Array.from(whitelistMatched),
        blacklistMatched: Array.from(blacklistMatched),
    };
}
